...
const createUser = async (req, res) => {
  ...
  try {
    ...
    if (!newUser.email) {
      ...
      const message = `Welcome to CarHealth!\nTo proceed with login you have to verify first\nPlease click on the link to verify your email: \n\n ${verificationURL}`;
...
const loginUser = async (req, res) => {
  ...
  try {
    ...
    if (user.isLocked) {
      return res.json({
        success: false,
        message: `Account is locked. It will be unlocked at ${user.lockUntil}.`,
    ...
    const token = jwt.sign(
      { id: user._id, isAdmin: user.isAdmin }, // ✅ FIXED: Using user.isAdmin instead of user.role
      process.env.JWT_SECRET,
      { expiresIn: "1h" }
    );
    const cookieOptions = {
...
const forgotPassword = async (req, res) => {
  ...
  try {
    ...
    // Send the email
    const message = `Welcome back to CarHealth.\nThis is password reset email.\nTo reset you password follow the link: \n\n ${resetUrl}`;
